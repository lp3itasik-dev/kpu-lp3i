        ...
        @if (count($candidates_non_organization) > 0)
            <section class="max-w-7xl mx-auto space-y-4 sm:px-6 lg:px-8">
                <header class="max-w-lg mx-auto text-center space-y-1">
                    ...
                    <p class="text-sm text-gray-600">Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusamus deserunt cumque excepturi ex praesentium corrupti eveniet sed sint quibusdam reiciendis?</p>
                </header>
                <div class="grid grid-cols-2 md:grid-cols-3 gap-5">
                    @foreach ($candidates_non_organization as $candidate_non_organization)
                        <form action="{{ route('voting.store') }}" method="POST" class="bg-white shadow-md rounded-lg text-center space-y-1 p-4">
                            @csrf
                            <div class="h-32 bg-gray-200"></div>
                            <h2 class="text-lg font-bold">{{ $candidate_non_organization->name }}</h2>
                            <h3 class="text-md font-medium">{{ $candidate_non_organization->organization->name ?? 'Unknown' }}</h3>
                            <p class="text-sm text-gray-700">{{ $candidate_non_organization->description }}</p>
                            <input type="text" name="candidate_id" value="{{ $candidate_non_organization->id }}">
                            <input type="text" name="card_vote_id" value="{{ $cardvote->id }}">
                            <button type="submit" class="inline-block bg-sky-500 hover:bg-sky-600 text-white px-5 py-2 text-sm font-medium rounded-xl">Pilih</button>
                        </form>
                    @endforeach
                </div>
            </section>
