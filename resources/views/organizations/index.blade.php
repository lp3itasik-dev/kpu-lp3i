                ...
                <div class="p-0 " style="width:100%;overflow-x:auto;">
                    <div class="flex flex-col sm:flex-row justify-between items-center mb-4">
                        <div class="md:mt-0 sm:flex sm:space-x-4 w-full">
                            <!-- Form untuk entries -->
                            <x-show-entries :route="route('organizations.index')" :search="request()->search">
                            </x-show-entries>
                        </div>
                                ...
                                @forelse($organizations as $key => $organization)
                                    <tr class="bg-white border dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="px-6 py-4 text-center bg-gray-100">
                                            {{ $organization->program->name ?? 'Unknown' }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            {{ $organization->is_active ? 'Active' : 'Non Active' }}
                                        </td>
                                        <td class="px-6 py-4 text-center flex flex-col lg:flex-row">
                                            <a href="{{ route('organizations.edit', $organization->id) }}" class="mr-2 hover:bg-amber-100 text-amber-600 pr-3 pl-3 py-[11px] rounded-xl text-xs border-2 border-dashed border-amber-600 mb-5"><i class="fa-solid fa-edit"></i></a>
                                            <form action="{{ route('organizations.destroy', $organization->id) }}" method="POST"
                                                class="inline"
                                                onclick="return confirm('Are you sure you want to delete this item?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"class="lg:mt-0 border-2 border-dashed border-red-500 text-red-500 hover:bg-red-100 px-3 py-1 rounded-xl h-10 w-10 text-xs"><i
                    ...
                    <div class="mt-4">
                        @if ($organizations->hasPages())
                            {{ $organizations->appends(request()->query())->links('vendor.pagination.custom') }}
                        @else
                            <div class="flex items-center justify-between">
                                <nav class="flex justify-start">
                                    <div class="text-sm flex gap-1">
                                        ...
                                        <div>to</div>
                                        <div class="font-bold">{{ count($organizations) }}</div>
                                        <div>of</div>
                                        <div class="font-bold">{{ count($organizations) }}</div>
                                        <div>entries</div>
                                    </div>
